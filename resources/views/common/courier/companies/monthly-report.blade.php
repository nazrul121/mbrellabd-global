...
<?php 
    $statusIDs = $allStatus = $allZone= $totalReturnCost =$orders = [];
...
// dd(date('d M,Y',strtotime($startDate)), date('d M, Y',strtotime($endDate)));
...
<div class="row">
    <div class="col-12">
        <div class="card">
            <form class="card-header" id="searchForm" action="{{route('common.courier.monthly-report',$courier_company->id)}}">@csrf
                ...
                <div class="form-group d-inline">
                    <div class="radio radio-success d-inline">
                        <input type="radio" name="durationType" id="dateBase" value="date">
                        <label for="dateBase" class="cr">Date wize report</label>
                    </div>
                </div>
                <div class="form-group d-inline">
                    <div class="radio radio-success d-inline">
                        <input type="radio" name="durationType" id="monthBase" value="month" checked="" >
                ...
                <div id="printableArea">
                    ...
                    <div class="responsive-table">
                        <table class="table table-hover table-bordered bg-white text-center" style="width:100%">
                            <thead>
                                <tr>
                                    <th class="text-center" colspan="{{$status->count() + $courier_company->courier_zones()->count() + $courier_company->courier_zones()->count() + 8}}">
                                        ...
                                        <h5><b>Courier order Report</b> (Month wise)</h5>
                                        <h6>Report Date: From  <b>{{date('M Y',strtotime($startDate))}}</b> To <b>{{date('M Y',strtotime($endDate))}}</b> </h6>
                            ...
                            <tbody>
                                @foreach($bundles as $key=>$bundle)
                                <tr class="row_ bunleTr">
                                    <td class="text-left">{{$key+1}}</td>
                                    <td class="text-left">{{date("F", strtotime("2023-$bundle->month-01"))}} ({{$bundle->month}}), {{ $bundle->year}} </td>
                                    
                                    <td> 
                                        @php
                                            $prices = $qtys = $orderIDs =[];

                                            $startDate = Carbon\Carbon::createFromDate($bundle->year, $bundle->month, 1);
                                            $endDate = $startDate->copy()->endOfMonth();

                                            // dd($startDate, $endDate);
                                            $query = DB::table('courier_company_orders')
                                                ->where('courier_company_id', $courier_company->id)
                                                ->whereBetween('created_at', [$startDate, $endDate]);


                                            foreach($query->get() as $courierOrder){
                                    ...
                                    @foreach($status as $sta)
                                        @php
                                            ...
                                            if($sta->relational_activity=='return'){
                                                foreach($courier_company->courier_zones()->get() as $key=>$zone){
                                                    $courier_company_orders = \App\Models\Courier_company_order::
                                                        where(['courier_company_id'=>$courier_company->id,'courier_zone_id'=>$zone->id])
                            ...
                            <tfoot class="bg-light text-center">
                                <tr>
                                    ...
                                    <td>{{array_sum($totalOrders)}}</td>
                                    <td>{{array_sum($totalPrices)}}</td>
                                    <td>{{array_sum($totalQtys)}}</td>

                                                    ...
                                                    @foreach($payments->get() as $payment)
                                                        ...
                                                        <span class="text-secondary"> Paid Amount: <span>{{ $order->country->currencySymbol.$payment->amount }} 
                                                            on <small>{{ date('M d,y h:ia',strtotime($payment->created_at)) }} via
                                                                @if($payment->payment_type_id==null) Not taken @else {{ $payment->payment_type->title }} 
                                        ...
                                        <td style="width:60%">
                                            ...
                                            @else
                                                ...
                                                @foreach($payments->get() as $payment)
                                                    ...
                                                    <span class="text-secondary"> Paid Amount: <span>{{ $order->country->currencySymbol.' '.$payment->amount }} on <small>{{ date('M d,y h:ia',strtotime($payment->created_at)) }} via @if($payment->payment_type_id==null) Not taken @else {{ $payment->payment_type->title }} @endif </small> (Transaction: {{ $payment->status }}) </span> <br>

...
<p class="alert alert-info">
    Order ID: #<code>{{ $order->id }}</code>, &nbsp;  Invoice Number: #<code>{{ $order->invoice_id }}</code>, &nbsp; Transaction ID: #<code>{{ $order->transaction_id }}</code>
</p>
...
@if($statusAction=='continue')
    ...
    <div class="collapse" id="changeProcedure">
        <form class="bg-light p-3 mb-4" id="changeStatus" method="get" action="{{ route('common.order.delivery-process',$order->id) }}"> @csrf
            <div class="form-group row">
                ...
                <select name="status" class="form-control">
                    ...
                    @foreach ($order_status as $ostatus)
                        ...
                        <option data-activity="{{ $ostatus->relational_activity }}" @if($oStatusID==$ostatus->id)disabled @endif value="{{ $ostatus->id }}"> {{ $ostatus->id.'. '.$ostatus->title }} - {{ $ostatus->description }}</option>
                    @endforeach
                </select>
...
@else
    <p class="text-danger alert alert-danger">The order status already been updated as <b class="text-info">{{ $order->order_status->title }}</b></p>
@endif
<br>
...
<script>
    $(function(){
        ...
        let table = $('.orderStatusChangeTable').DataTable({
            ...
            ajax: "{{route('common.order.delivery-process',$order->id)}}",
            order: [ [0, 'desc'] ],
            columns: [
                {data:'id'},
        ...
        $('[name=status]').on('change',function(){
            ...
            if(status_id !=''){
                if(relationalData=='ask-for-payment'){
                    $.get( "{{route('common.check-order-payment',$order->id)}}", function( data ) {
                        if(data[0] == true){

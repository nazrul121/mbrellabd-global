        ...
        <div class="featured-collection-section mt-100 home-section overflow-hidden">
            <div class="container">
                <div class="section-header">
                    <h2 class="section-heading">{{$highlight->title}}</h2>
                </div>
                ...
                <div class="product-container position-relative">
                    <div class="common-slider" data-slick='{
                        ...
                        @foreach ($highlight->products()->select('products.id','products.title','products.slug','products.thumbs','products.sale_price','products.net_price')->inRandomOrder()->limit(12)->get() as $key=>$product)
                        <?php
                            ...
                            <div class="product-card">
                                <div class="product-card-img">
                                    <a class="hover-switch" href="{{ route('product',[app()->getLocale(), $product->slug]) }}">
                                        ...
                                        {{-- {{ url('storage').'/'.$product->thumbs }} --}}
                                        <img class="primary-img img{{ $product->id }}" src="{{ url($product->thumbs) }}" alt="{{$product->title}}">
                                    </a>

                                    @if($percent >0)
                                        <div class="product-badge">
                                            <span class="badge-label badge-percentage rounded">{{ number_format($percent,1) }} % off</span>
                                        </div>
                                    @endif
                                    ...
                                    <div class="product-card-action product-card-action-2 justify-content-center">
                                        <a href="#quickview-modal" class="action-card action-quickview" data-bs-toggle="modal" data-product_id="{{ $product->id }}">
                                        ...
                                        <a href="javaScript:;" class="action-card action-wishlist" data-product_id="{{ $product->id }}" data-key="{{ $key }}">
                                        ...
                                        @if(request()->get('addToCart')=='1')
                                            <a href="javaScript:;" class="action-card action-addtocart" data-product_id="{{ $product->id }}" data-variation="{{$product->product_variation_options()->count()}}">
                                ...
                                <div class="product-card-details">
                                    ...
                                    <input type="hidden" class="default_thumb{{ $product->id }}" value="{{ $product->thumbs }}">

                                    <h3 class="product-card-title">
                                        <a href="{{ route('product',[app()->getLocale(), $product->slug]) }}">{{ $product->title }}</a>
                                    </h3>
                                    <div class="product-card-price">
                                        <span class="card-price-regular">{{ Session::get('user_currency')->currencySymbol }}</span> {{ number_format(product_price($product->id, $product->sale_price), $formatNumber)}}</span>
                                        @if($percent >0) <span class="card-price-compare text-decoration-line-through">{{ Session::get('user_currency')->currencySymbol }} {{ number_format($old,$formatNumber)}}</span> @endif
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        {{-- <div class="new-item" data-aos="fade-up" data-aos-duration="300">
                            <div class="product-card mt-5">
                                <div class="product-card-img">
                                    <a class="hover-switch" href="{{ route('highlight-products',[app()->getLocale(),$highlight->id])}}">
                                        <img class="primary-img" src="{{url('storage/'.$highlight->photo)}}" alt="{{$highlight->title}}">
                                    </a>
                                </div>
                                <div class="product-card-details text-center">
                                    <h3 class="product-card-title"><a href="{{ route('highlight-products',app()->getLocale(), $highlight->id)}}">View All</a>  </h3>
                                </div>
                            </div>
                        </div> --}}
                    </div>
...
<script>
    ...
    $('.action-addtocart, .addtocart-btn').on('click',function(){
        ...
        if(variation >0){
            ...
            $.get("/{{ app()->getLocale() }}/add-to-cart?qty=1&id="+id, function(data, status){
                if(data[1]=='success') {
                    $('#notice').fadeIn('slow');
